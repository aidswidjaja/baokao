...
<script>
const initial_request_$id = '$google_drive_folder_id' // TODO: MATCH WITH REPO

folder_arr_$id = [];

search_ids_$id(initial_request_$id); // search repo first!

function search_ids_$id(repo_id) { // get all IDs in parent, search for folders and store every folder ID
    const search_ids_request_$id = "https://www.googleapis.com/drive/v3/files?q='" + repo_id + "'+in+parents&orderBy=name&key=" + google_drive_api_key;
    fetch(search_ids_request_$id)
        .then(response => response.json())
        .then(data => {
            for (const file of data.files) { 

                // if folder, record ID and search again!
                // otherwise, get yeeted

                if (data.mimeType = "application/vnd.google-apps.folder") {
                    folder_arr_$id.push(file.id);
                    search_ids_$id(file.id);
                }
                
            }
            file_search_$id(folder_arr_$id); // for each folder in repo, search for files with match query
        }
    )
}

const file_explorer_$id = document.getElementById('$file_explorer_id');

function file_search_$id(folder_arr) {
    console.log("EWQIWEHRUIQEWOHR")
    for (const id of folder_arr) {
        const file_search_request_$id = "https://www.googleapis.com/drive/v3/files?q=fullText+contains+" + "'$query'" + "+and+trashed=false+and+'" + id + "'+in+parents&key=" + google_drive_api_key;
    fetch(file_search_request_$id)
        .then(response => response.json())
        .then(data => {
            for (const file of data.files) { // hello i am stolen from get.js
